...
char menu_items[][60] = {" 	 Continue - Normal boot"," 	 Boot - boot in defferent modes"," 	 Fixs - solve start-up problems"," 	 Mount - Montar puntos de particiones"," 	 Backup - Copy and restore files"," 	 Extras - Molecular, Vitashell, ..."};

char menu_options [][8][28] = {  {"exit"} , {"Reboot","IDU ON","IDU OFF (DEMO MODE)"} , {"Delete id.dat","Delete act.dat","Delete ux0:tai/config.txt","Delete ur0:tai/config.txt","Clear log config in vita"} , {"Mount MemCard","Umount MemCard"} , {"Backup Activation","Restore Activation","Copy ur0 tai","Restore ur0 tai","Copy ux0 tai","Restore ux0 tai"} , {"Molecular to NEAR","Restore NEAR","Copy VITASHELL in NEARMOD","CID and IMEI num","Uninstall RECOVERY","Clean LOG"}  };
			...
			if (pad.buttons == SCE_CTRL_CROSS) {
				switch (selected){//ADD CHECKS TO LOG
					case 0://Continue
						switch (sub_selected){
							case 0:
								sprintf(con_data, "Continuing the start... ");
					...
					case 1://Boot
						switch (sub_selected){
							...
							case 2://IDU Disable
								...
								sprintf(con_data, "IDU disable (DEMO MODE): %d \n", ret);
								strcat(log_text,con_data);
								break;
					...
					case 2:
						switch (sub_selected){
							...
							case 1://Delete act.dat
								...
								sprintf(con_data, "Erased act.dat: %d rebooting in 5s", ret);
								strcat(log_text,con_data);
                                                                select_menu();
							...
							case 2://Delete tai config UX0
								...
								sprintf(con_data, "Erased config ux0:tai %d rebooting in 5s", ret);
								strcat(log_text,con_data);
                                                                select_menu();
								sceKernelDelayThread(5 * 1000 * 1000);
                                                                scePowerRequestColdReset();
								...
								sprintf(con_data, "Erased config ur0:tai %d rebooting in 5s", ret);
								strcat(log_text,con_data);
                                                                select_menu();
								sceKernelDelayThread(5 * 1000 * 1000);
                                                                scePowerRequestColdReset();
					...
					case 3:
						switch (sub_selected){
							case 0://Mount mem card
								...
								sprintf(con_data, "Mounting MemCard: %d \n", ret);
								strcat(log_text,con_data);
								break;
							case 1://Unmount mem card
								ret = vshIoUmount(0x800, 0, 0, 0);
								sprintf(con_data, "Unmounting MemCard: %d \n", ret);
								strcat(log_text,con_data);
								break;
						...
						switch (sub_selected){
							case 0://Copy activation
								...
								copyFile("ur0:/user/00/np/myprofile.dat" ,"ux0:/Backup_act/myprofile.dat");
                                                                sprintf(con_data, "copying activation files: OK! \n");
								strcat(log_text,con_data); } 
                                                                else 
							...
							case 1://Restore activation
								...
								copyFile("ux0:/Backup_act/myprofile.dat" ,"ur0:/user/00/np/myprofile.dat");
                                                                sprintf(con_data, "restoring activation files: %d rebooting in 5s", ret);
								strcat(log_text,con_data);
                                                                select_menu();
								sceKernelDelayThread(5 * 1000 * 1000);
                                                                scePowerRequestColdReset();
							...
							case 2://Copy tai config UR0
								...
								sprintf(con_data, "copying config ur0:tai : Ok! XD  \n");
								strcat(log_text,con_data); } 
                                                                else 
								...
								sprintf(con_data, "Restoring config ur0:tai %d rebooting in 5s", ret);
								strcat(log_text,con_data);
                                                                select_menu();
								sceKernelDelayThread(5 * 1000 * 1000);
                                                                scePowerRequestColdReset();
								...
								sprintf(con_data, "Copying config ux0:tai : Ok! XD  \n");
								strcat(log_text,con_data); } 
                                                                else 
							...
							case 5://Restore tai config UX0 
								...
								sprintf(con_data, "Restoring config ux0:tai %d rebooting in 5s", ret);
								strcat(log_text,con_data);
                                                                select_menu();
								sceKernelDelayThread(5 * 1000 * 1000);
                                                                scePowerRequestColdReset();
					...
					case 5:
						switch (sub_selected){
							...
							case 2://Molecular x Vitashell
							        ...
							        printf("changing MOLECULAR for VITASHELL!!\n");
								{for (i = 0; i < 15; i++) {
		                                                printf(".");
								...
								strcat(log_text,con_data); }
                                                                ...
                                                                {sprintf(con_data, "First make a copy of Molecular in NEAR... :(  \n");
								strcat(log_text,con_data);
	                                                        }
                                                                break;
							...
							case 4://Uninstall RECOVERY
								...
								{printf("Install HomeRecovery with the VPK first");}
								break;
